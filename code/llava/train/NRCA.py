...
class RatioGapTwo(LLaVATrainer):
        ...
        for epoch in range(epochs_trained, num_train_epochs):
            ...
            for step, inputs in enumerate(epoch_iterator): # 开始循环
                ...
                with self.accelerator.accumulate(model): # automatically perform the gradient accumulation
                    mean = inputs['images'].mean()
                    std = inputs['images'].std()

                    # 根据图像的均值和标准差生成噪声
                    noise = torch.randn_like(inputs['images']) * std + mean

                    fake_images = ((noise + inputs['images']) / 2)

                    fake_input = {
                        'input_ids': inputs['input_ids'],
                        'labels': inputs['labels'],
                        'attention_mask': inputs['attention_mask'],
                        'images': fake_images.float()
                    }
                    tr_loss_step = self.ratio_gap_two_training_step(model, inputs,fake_input)
