...
def load_pretrained_model(model_path, model_base, model_name,
                          load_8bit=False, load_4bit=False, device_map="auto", device="cuda", use_flash_attn=False, sparse_method='normal', **kwargs):
    ...
    if 'llava' in model_name.lower():
        ...
        if 'lora' in model_name.lower() and model_base is not None:
            ...
            if 'mpt' in model_name.lower():
                ...
                # LlamaTokenizer 是一种分词器，用于将输入文本转换为模型可处理的数字表示，包含了多个特殊标记（如 <s>、</s> 和 <unk>）
                tokenizer = AutoTokenizer.from_pretrained(model_path, use_fast=False)

                if sparse_method=='SparseVLM':
                    print("Start SparseVLM Inference...")
