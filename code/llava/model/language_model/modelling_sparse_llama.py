...
class LlamaDynamicvitModel(LlamaModel):
    def __init__(self, config: LlamaConfig, pruning_loc=[2, 6, 15, 19]):
        ...
        self.vocab_size = config.vocab_size # 定义了模型的词汇表大小和嵌入层
    ...
    def forward(
        ...
        num_token = []
        B, L, _ = hidden_states.shape # Batch Length Embedding
        idx_sprase_layer = 0 # idx_sprase_layer 是一个用于追踪稀疏层索引的变量（初始为0）
        out_pred_prob = None
        ...
        v_t = hidden_states[:, v_token_start: text_token_start, :] # 1 x 576 x 4096
        t_t = hidden_states[:, text_token_start:, :] # 1 x 53 x 4096
        ...
        for layer_idx, decoder_layer in enumerate(self.layers):
            ...
            if layer_idx in self.pruning_loc and len(pre_prompt_length_list) != 0 and hidden_states.shape[1] != 1:
                ...
                pred_score_vis, s_flag, relation_vis_text = attn_postprocess_rank(attn_logits, v_token_start,
                                                                                  v_token_num, \
                                                                                  text_token_start, t_token_idx,
                                                                                  scale=scale, bias=bias)  # B, L_v
                ...
                if s_flag:
                    ...
                    merge_token_idx_stage1 = torch.where(pred_score_vis == 0)[1] # 抓取设置为0的 index
                    merge_token_stage1 = relation_vis_text[0][merge_token_idx_stage1] # 抓取他们和Text 的相似性
                    merge_token_num_stage1 = int(merge_token_idx_stage1.shape[0] * 0.3) + 1  # Top 30% # 选择token的比例
                    merge_token_stage2_idx = merge_token_stage1.topk(merge_token_num_stage1)[1] # 抓取出topk的token idx

                    merge_token_stage2 = total_sparse_token[:, merge_token_stage2_idx, :] #抓取出topk的token的embedding
                    ...
                    select_token_idx = torch.where(policy == 1)[1].unsqueeze(0)  # B, L_new
                    layer_outputs = (
                    batch_index_select(layer_outputs[0], select_token_idx), layer_outputs[1])  # B, L, C
                    ...
                    # print(layer_idx, v_token_num)
                    text_token_start = v_token_start + v_token_num
                # print('Layer Visual Token' + str(layer_idx) +str(v_token_num))
...
class LlamaDynamicvitAttention(LlamaAttention):
    def forward(
        ...
        if policy is None:
            ...
            attn = (query_states @ key_states.transpose(-2, -1)) / math.sqrt(
                self.head_dim)  # 因为llama的attention代码没有缩放因子，所以我就不缩放了
...
class LlamaDynamicvitFlashAttention2(LlamaFlashAttention2):
    def forward(
        ...
        query_states = query_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1,
                                                                                              2)  # torch.Size([2, 32, 687, 128]) ,32个头
        ...
        if len(position_ids[0]) == 1:
            position_ids = torch.tensor([[past_key_value.get_usable_length(kv_seq_len, self.layer_idx)]],
...
class LlamaDynamicvitDecoderLayer(LlamaDecoderLayer):
    def __init__(self, config: LlamaConfig, layer_idx: int):
        ...
        self.self_attn = LLAMA_ATTENTION_CLASSES[config._attn_implementation](config=config, layer_idx=layer_idx) # 注意力
