...
def scaled_dot_product_attention_with_policy(query, key, value, policy, attn_mask=None, dropout_p=0.0, is_causal=False,
                                             scale=None):
...
def cluster_and_merge(x, cluster_num):
    ...
    x1 = ein.rearrange(x, "b l r -> b l () r") # x1 和 x2 是通过 einops 重排后的 x，用于计算任意两个特征点之间的欧氏距离
    x2 = ein.rearrange(x, "b l r -> b () l r")
    distance = (x1 - x2).norm(dim=-1, p=2) # distance 是一个三维张量，存储了所有特征点之间的距离，形状为 (B, N, N)
    dist_matrix = distance / (C ** 0.5) # dist_matrix = distance / (C ** 0.5)
    # get local density
