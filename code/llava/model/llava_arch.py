...
class LlavaMetaForCausalLM(ABC):
    # 作用是：将图像利用clip生成visual features（特征维度和token embedding一致）
    # 将word对应的token embedding和visual features 合并在一起，
    # visual features插入的位置就是input_ids中<image>的位置。
    @abstractmethod
    def get_model(self):
        pass
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    cur_image_features = image_features[cur_image_idx]
                    cur_image_idx += 1
                    cur_new_input_embeds.append(cur_image_features) # 576 x 4096
    ...
    def prepare_sparse_inputs_labels_for_multimodal(
        ...
        _attention_mask = attention_mask
        if attention_mask is None:
            attention_mask = torch.ones_like(input_ids, dtype=torch.bool) # 全True
        else:
            attention_mask = attention_mask.bool()
        if position_ids is None:  # 用input_ids的长度来填充position_ids
            position_ids = torch.arange(0, input_ids.shape[1], dtype=torch.long, device=input_ids.device) # 从 0开始 顺序排列
        if labels is None:
            labels = torch.full_like(input_ids, IGNORE_INDEX) # labels = [-100,-100]
        ...
        for batch_idx, cur_input_ids in enumerate(input_ids):
            ...
            cur_input_embeds = self.get_model().embed_tokens(
                torch.cat(cur_input_ids_noim))  # 相当于不对<image>这个标识符进行embedding，torch.Size([92, 4096])
            cur_input_embeds_no_im = torch.split(cur_input_embeds, split_sizes,
                                                 dim=0)  # 再将embedding分成<pre_prompt>、<question>两部分
            ...
            for i in range(num_images + 1):
                ...
                if i < num_images:
                    ...
                    cur_new_input_embeds.append(
                        cur_image_features)  # 之前不是没把那个<image>标识符给embedding嘛，其实就是为了这一步，把<image>标识符换成真正的image_features
                    cur_new_labels.append(
                        torch.full((cur_image_features.shape[0],), IGNORE_INDEX, device=cur_labels.device,
                                   dtype=cur_labels.dtype))
        ...
        # Truncate sequences to max length as image embeddings can make the sequence longer
        tokenizer_model_max_length = getattr(self.config, 'tokenizer_model_max_length', None)
        if tokenizer_model_max_length is not None: # 做截断操作
            new_input_embeds = [x[:tokenizer_model_max_length] for x in new_input_embeds]
            new_labels = [x[:tokenizer_model_max_length] for x in new_labels]
        ...
        for i, (cur_new_embed, cur_new_labels) in enumerate(zip(new_input_embeds, new_labels)):
            ...
            if getattr(self.config, 'tokenizer_padding_side', 'right') == "left":
                ...
                new_input_embeds_padded.append(torch.cat(( # 填充 new_input_embeds_padded：
                    cur_new_embed,
                    torch.zeros((max_len - cur_len, cur_new_embed.shape[1]), dtype=cur_new_embed.dtype,
                                device=cur_new_embed.device)
                ), dim=0))
        ...
        return None, position_ids, attention_mask, past_key_values, new_input_embeds, new_labels, image_features.shape[
            1], token_length_list, pre_prompt_length_list
        # image_features.shape[1], token_length_list, pre_prompt_length_list 为新增的！！
